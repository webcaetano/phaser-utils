...
/***/ function(module, exports, __webpack_require__) {
	...
	self.pointBetweenPorcent = function (point1, point2, por) {
		return self.radPos(point1, self.angleBetweenRad(point1, point2), self.dist(point1, point2) * por);
	};
	...
	// 	for(let i in assets.atlas) {
	// 		game.load.atlasJSONHash(i, assets.atlas[i].image, assets.atlas[i].jsonUrl, assets.atlas[i].json);
	// 	}
	// 	for(let i in assets.images) game.load.image(i, assets.images[i]);
	// 	for(let i in assets.sprites) game.load.spritesheet(i, assets.sprites[i].image, assets.sprites[i].width, assets.sprites[i].height, assets.sprites[i].frames);
	// 	for(let i in assets.audio) game.load.audio(i, assets.audio[i]);
	// }
